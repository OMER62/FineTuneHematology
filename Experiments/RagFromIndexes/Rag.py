...
retriever1 = VectorStoreIndex.from_vector_store(vector_store=vector_store1, embed_model=embed_model1).as_retriever(similarity_top_k=5)

# Second index setup
pc2 = Pinecone(api_key=os.environ.get("PINECONE_API_KEY"))  # Assuming the same API key can be used
index2 = pc2.Index(name="another-index-name")
...
retriever2 = VectorStoreIndex.from_vector_store(vector_store=vector_store2, embed_model=embed_model2).as_retriever(similarity_top_k=5)
...
pipeline.add_link("retriever1", "reranker", dest_key="nodes1")
pipeline.add_link("retriever2", "reranker", dest_key="nodes2")
